static void
vm_action_cb(void *data, zrpc_handle h)
{
	zwin *zwin = data;
...
/*
	...
	if (!zrpc_VM_save(uuid, zwin->zcon, vm_action_cb, zwin))
	{
...
/*
	...
	if (!zrpc_VM_destroy(uuid, zwin->zcon, vm_action_cb, zwin))
	{
		elm_label_label_set(zwin->zmain->status, "Connection to zrpc failed!");
		evas_object_show(zwin->zmain->notify);
		return;
...
vm_shutdown_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (!zrpc_VM_shutdown(uuid, zwin->zcon, vm_action_cb, zwin))
	{
		elm_label_label_set(zwin->zmain->status, "Connection to zrpc failed!");
		evas_object_show(zwin->zmain->notify);
		return;
...
vm_reboot_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (!zrpc_VM_reboot(uuid, zwin->zcon, vm_action_cb, zwin))
	{
		elm_label_label_set(zwin->zmain->status, "Connection to zrpc failed!");
		evas_object_show(zwin->zmain->notify);
		return;
...
vm_pause_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (!zrpc_VM_pause(uuid, zwin->zcon, vm_action_cb, zwin))
	{
		elm_label_label_set(zwin->zmain->status, "Connection to zrpc failed!");
		evas_object_show(zwin->zmain->notify);
		return;
...
vm_unpause_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (!zrpc_VM_unpause(uuid, zwin->zcon, vm_action_cb, zwin))
	{
		elm_label_label_set(zwin->zmain->status, "Connection to zrpc failed!");
		evas_object_show(zwin->zmain->notify);
		return;
...
vm_info_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	vmitem *item;
	zrpc_vm *vm;
	char *os_icon, *state, tmp[PATH_MAX];
	Eina_List *l = NULL;
	zrpc_disk *d;
	zrpc_vif *v;
	int cpus;
